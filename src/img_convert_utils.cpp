...
bool ImgConvertUtils::bgr_mat_to_nv12(const cv::Mat &bgr, uint8_t *nv12) {
  if (!nv12)
    return false;
  if (bgr.empty() || bgr.type() != CV_8UC3)
    return false;
  // bgr24_to_nv12_neon 假设每行 BGR 连续；Mat 带 step 间隙时 bgr.data 线性扫描会越界
  cv::Mat cont = bgr.isContinuous() ? bgr : bgr.clone();
  const int width = cont.cols;
  const int height = cont.rows;
  if (width <= 0 || height <= 0)
    return false;
  bgr24_to_nv12_neon(cont.data, nv12, width, height);
  return true;
}
